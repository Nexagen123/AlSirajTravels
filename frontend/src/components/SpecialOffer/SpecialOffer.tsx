...
export default function SpecialOffer() {
    ...
    return (
        <div className="siraj-section py-16 px-4">
            <div className="max-w-7xl mx-auto">
                {/* Heading Section */}
                <div className='relative w-fit mt-4 mb-12 text-center mx-auto'>
                    <h2 className='relative text-(--siraj-blue-dark) text-3xl sm:text-4xl z-1 text-center font-bold'>Special Offers</h2>
                    <div className='absolute bottom-1 left-0 w-full h-[35%] bg-(--siraj-gold-light)/60 rounded-md z-0'></div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    {loading ? (
                        // Skeleton Loaders
                        [1, 2, 3].map((n) => (
                            <div key={n} className="siraj-card p-2 animate-pulse">
                        ...
                        offers.map((offer) => (
                            <div key={offer._id} className="group siraj-card p-2 transition-all hover:-translate-y-1">
                                <div
                                ...
                                <div className="py-4 px-2 flex justify-between items-center">
                                    ...
                                    <button onClick={() => setPreviewImage(offer.image)} className="text-xs font-bold text-(--siraj-blue) hover:text-(--siraj-gold) transition-colors">
                                        VIEW DETAIL
                                    </button>
                                </div>
                            </div>
